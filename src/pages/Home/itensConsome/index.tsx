...
function Consumido() {
  ...
  const addItemConsumo = (data: { nameItem: string, quantidade: number }) => {
    ...
    const verifica = allItensConsumo.find((p: any) => p.nameItem === data.nameItem)
    if (!verifica) {
      ...
      const mod = allItensConsumo.filter((obj: any) => obj.nameItem !== verifica.nameItem)
      mod.push({
        nameItem: data.nameItem,
  ...
  function atualizaIdiceLista(id: string, metod: string) {
    const allItensConsumo = buscarItemsConsumo()
    const indice = allItensConsumo.find((p: any) => p.id === id)
    const mod = allItensConsumo.filter((obj: any) => obj.id !== id)
  ...
  return (
    <div style={{ height: "90vh", width: "100vw" }}>
      <Form<initialValueCart, typeCart> control={control} style={{ width: '100%' }} onSubmit={(data) => addItemConsumo(data.data)}>
      ...
      <div className="ag-theme-alpine" style={{ height: '80vh', width: '100%', justifyContent: 'center', display: 'flex' }}>
